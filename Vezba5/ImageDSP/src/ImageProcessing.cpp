...
void imageProcessingFun(const QString& progName, QImage* const outImgs, const QImage* const inImgs, const QVector<double>& params) 
{
	...
	*outImgs = *(new QImage(X_SIZE, Y_SIZE, inImgs->format()));
	...
	{	
		...
		/* TODO */
		performDCT(Y_buff, X_SIZE, Y_SIZE, 256);
	...
	else if (progName == "DCT + IDCT") 
	{
		...
		/* TODO */
		performDCTandIDCT(Y_buff, X_SIZE, Y_SIZE, 32);
	}
	...
	else if (progName == "DCT Quantization") 
	{
		...
		/* TODO */
		uchar * pre_quant = new uchar[inImgs->width()*inImgs->height()];
		for (int i = 0; i < X_SIZE; i++)
		{
			for (int j = 0; j < Y_SIZE; j++)
			{
				pre_quant[j*X_SIZE + i] = Y_buff[j*X_SIZE + i];
			}
		}
		performDCTQuantization(Y_buff, X_SIZE, Y_SIZE, N);
		double MSE = 0;
		for (int i = 0; i < X_SIZE; i++)
		{
			for (int j = 0; j < Y_SIZE; j++)
			{
				MSE += (Y_buff[j*X_SIZE + i] - pre_quant[j*X_SIZE + i])*(Y_buff[j*X_SIZE + i] - pre_quant[j*X_SIZE + i]);
			}
